...
* 创建一个用户
```shell
mysql> grant all on *.* to duoyi@'%' identified by '123456';  #注意这个%代表是任意主机的意思，生产环境不建议使用可以组合使用例如192.168.1.%
...
```shell
mysql> grant insert,update on *.* to duoyichen@'%' identified by '123456';  #先创建一个具有insert和update权限的用户
Query OK, 0 rows affected (0.00 sec)
...
mysql>
mysql> show grants for duoyichen@'%';   #查看用户权限
+----------------------------------------------------------------------------------------------------------------+
| Grants for duoyichen@%                                                                                            |
+----------------------------------------------------------------------------------------------------------------+
| GRANT INSERT, UPDATE ON *.* TO 'duoyichen'@'%' IDENTIFIED BY PASSWORD '*6BB4837EB74329105EE4568DDA7DC67ED2CA2AD9' |
+----------------------------------------------------------------------------------------------------------------+
1 row in set (0.00 sec)

mysql>
mysql> revoke update on *.* from duoyichen@'%';  #移除update
Query OK, 0 rows affected (0.00 sec)

mysql>
mysql> show grants for duoyichen@'%';    #再次查看用户权限
+--------------------------------------------------------------------------------------------------------+
| Grants for duoyichen@%                                                                                    |
+--------------------------------------------------------------------------------------------------------+
| GRANT INSERT ON *.* TO 'duoyichen'@'%' IDENTIFIED BY PASSWORD '*6BB4837EB74329105EE4568DDA7DC67ED2CA2AD9' |
+--------------------------------------------------------------------------------------------------------+
1 row in set (0.00 sec)
...
```shell
mysql> insert into users(name) values('duoyi');
Query OK, 1 row affected (0.01 sec)
...
```shell
mysql> insert into users(name) values('duoyi3');           
...
mysql> update users set name='duoyi1' where id=2;     
...
root@template /backup/mysql 21:05:46 # mysqldump -uroot -p7758521 duoyi > back1.sql
...
```shell
root@template /backup/mysql 21:09:02 # mysqldump -uroot -p7758521 -B duoyi > back2.sql 
...
18a19,26
> -- Current Database: `duoyi`
> --
> 
> CREATE DATABASE /*!32312 IF NOT EXISTS*/ `duoyi` /*!40100 DEFAULT CHARACTER SET utf8 */;
> 
> USE `duoyi`;                             
> 
> --
...
```
```shell
root@template /backup/mysql 16:29:15 # mysqldump -uroot -p7758521 -B test duoyi > back3.sql
mysql> drop database test;
Query OK, 0 rows affected (0.00 sec)

mysql> drop database duoyi;
Query OK, 1 row affected (0.00 sec)
...
mysql> \q
Bye
root@template /backup/mysql 16:39:37 # mysqldump -uroot -p7758521 duoyi users users2 > back4.sql
...
mysql>\q
Bye
root@template /backup/mysql 16:43:38 # mysql -uroot -p7758521 duoyi < back4.sql 
...
mysql> insert into users(name) values('chenduoyi');              
